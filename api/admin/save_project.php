...
if ($id > 0) {
    ...
    $stmt = $conn->prepare("UPDATE projects SET title = ?, description = ?, image_url = ?, video_url = ?, media_gallery = ?, display_order = ?, is_active = ? WHERE id = ?");
    $stmt->bind_param("sssssiii", $title, $description, $image_url, $video_url, $media_gallery, $display_order, $is_active, $id);
} else {
    // Insert new project
    $stmt = $conn->prepare("INSERT INTO projects (title, description, image_url, video_url, media_gallery, display_order, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)");
